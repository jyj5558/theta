...
```{r}
GDs <- data[c("Nucleotide_diversity_mean", "Watterson_theta_mean", "Tajima_D_mean", "Pop_heterozygosity_mean", "Froh_100kb", "Froh_1mb")]
...
tiff("Supplementary Figure 14.tif", units="px", width=2100, height=1500, res=300)
ggplot(data = sub_data, aes(x = Dim.1, y = Dim.2, colour = Pop_heterozygosity_mean, shape = IUCN_category)) + 
  geom_point(size = 2.5, alpha = 0.8, position = position_jitter(w=0.05)) + 
  theme_classic() +
  theme_classic(base_size = 16) +
  scale_color_continuous_divergingx(palette = "RdBu", l2=75, name = "Heterozygosity", mid = mean(sub_data$Pop_heterozygosity_mean)) +
  guides(shape = guide_legend(title = "IUCN category")) +
  xlab ("Dim.1 (17.4%)") + ylab ("Dim.2 (10.9%)") +
